...
sub insert_category {
	...
	$cat_sth->execute(prep_postgres_array(@{$range->{"breadcrumbs"}}));
	my $cat_id = $cat_sth->fetchrow_arrayref()->[0];
...
sub insert_product_category {
	...
	state $sth = do {
		my $sql = "INSERT INTO product_category (category, product_number) VALUES (?, ?)";
		$dbh->prepare($sql);
	};

	$sth->execute($cat_id, $range->{"product_id"});
}
